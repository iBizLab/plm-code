...
var z = (e, p, t) => (de(e, typeof p != "symbol" ? p + "" : p, t), t);
...
import { useControlController as _e, useNamespace as me, withInstall as ye } from "@ibiz-template/vue3-util";
import { ref as j, defineComponent as De, reactive as ve, watch as X, computed as Ie, nextTick as ge, resolveComponent as g, onMounted as we, onUnmounted as xe, withDirectives as Ee, createVNode as f, resolveDirective as be, isVNode as Ce, createTextVNode as Y } from "vue";
...
function Te(e, p) {
  const t = () => {
    ...
    Object.values(d.store.nodesMap).forEach((l) => {
      const y = p.state.expandedKeys.includes(l.data._id);
...
class ke extends le {
  ...
  async onCreated() {
    ...
    o.HIDDENNODEID && (this.hiddenNodeId = o.HIDDENNODEID);
  }
  initState() {
  ...
  async onRemoveTreeNode(t) {
    ...
    try {
      ...
      await fe(
        r.map(async (y) => {
          if (y.srfuf !== he.CREATE) {
            const K = o.clone();
            K[l] = y.srfkey, await await ibiz.hub.getApp(d.appId).deService.exec(
              d.appDataEntityId,
              "remove",
              K,
              r,
        ...
        data: r,
        default: "数据[".concat(r.map((y) => y.srfmajortext).join("、"), "]删除成功!")
  ...
  async onNodeDrop(t, o, s) {
    ...
    let y = !1;
    this.getNodeModel(l._nodeId).rootNode && (y = !0);
    const S = (l == null ? void 0 : l._id) !== ((F = t._parent) == null ? void 0 : F._id);
    let C = this.getNodeModel(o._nodeId);
    if (s === "inner" || ((P = o._parent) == null ? void 0 : P._id) !== ((T = t._parent) == null ? void 0 : T._id) || y) {
      const D = [];
      ...
      if (y && S)
        ...
        m && (D && D.forEach((_) => {
          ...
          var k, M;
          _.name && _.value && ((k = t._deData) != null && k.hasOwnProperty(
            _.name.toLowerCase()
    ...
    if (B.splice(B.indexOf(t), 1), s === "inner")
      ...
      let D = l._children.indexOf(o);
      s === "next" && (D += 1), l._children.splice(D, 0, t);
    }
    (s === "inner" || S) && (t._parent = l, t._nodeId = C.id, this.state.expandedKeys = this.calcExpandedKeys([l]));
    const { sortAppDEFieldId: A, sortDir: U, allowOrder: O } = C;
    if (O === !0) {
      if (!A)
        throw new ee(C, "缺少配置排序属性");
      const D = A.toLowerCase(), m = U === "ASC", _ = [...l._children].filter(
        (v) => v._nodeType === "DE"
      );
      m || _.reverse();
      const k = (v) => v + (100 - v % 100), M = (v) => v[D] || 0;
      let x;
      _.forEach((v, V) => {
        const R = v._deData;
        x === void 0 ? v === t && (V === 0 ? x = 100 : x = k(M(_[V - 1]._deData)), R[D] = x, d.indexOf(v) === -1 && d.push(v)) : (x >= M(R) && (R[D] = k(x), d.push(v)), x = M(R));
      });
    }
  ...
  setup() {
    ...
    e.evt.on("onAfterRefreshParent", (n) => {
      if (o.value) {
        ...
        o.value.updateKeyChildren(i._id, c), y();
      }
    ...
    const B = async (n, i) => {
      ...
      ibiz.log.debug("给树返回值", a), i(S(a)), y();
    };
    let A = !1;
    ...
    const F = (n, i) => {
      ...
      if (i.stopPropagation(), !O) {
        ...
        e.onTreeNodeClick(n, i), O = !0, setTimeout(() => {
          O = !1;
        }, 200);
      }
    }, P = (n, i) => {
    ...
    const D = g("IBizRawItem"), m = g("IBizIcon"), _ = (n, i, a, c) => {
      const N = [];
      return n.forEach((u) => {
        var L;
        ...
        if (u.showCaption && u.caption && (b.label = u.caption), u.sysImage && u.showIcon && (b.icon = f(m, {
          ...
          u.itemType === "ITEMS" && (L = u.detoolbarItems) != null && L.length && (b.children = _(u.detoolbarItems, i, a, c));
        N.push(b);
      }), N;
    }, k = async (n, i) => {
        ...
        return f("div", {
          ...
          default: ({
            ...
            return u ? E = f(g("iBizControlShell"), {
              ...
              onDblclick: (I) => P(c, I),
              onClick: (I) => F(c, I),
              onContextmenu: (I) => k(c, I),
              class: [t.b("node"), t.is("hidden", Object.is(e.hiddenNodeId, c._nodeId) && !e.isFilter.value), (L = N.sysCss) == null ? void 0 : L.cssName]
            }, [E, J(N), M(N, c)]);
          }
...
class Se {
  constructor() {
    z(this, "component", "IBizGroupTreeControl");
  }
}
const Ue = ye(
  H,
