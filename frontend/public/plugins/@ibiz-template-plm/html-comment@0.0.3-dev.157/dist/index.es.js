...
import { defineComponent as ke, ref as g, shallowRef as ot, watch as le, onBeforeUnmount as Ee, onMounted as ze, nextTick as U, createVNode as p, resolveComponent as q, createTextVNode as j, onUnmounted as xt, computed as lt, withDirectives as ct, resolveDirective as Re, h as dt, defineAsyncComponent as Pt } from "vue";
import { ScriptFactory as J, convertNavData as De, EditorController as Lt, ControllerEvent as Tt, getDeACMode as At, registerEditorProvider as Ht } from "@ibiz-template/runtime";
...
import { CoreConst as vt, debounce as gt, awaitTimeout as It, RuntimeError as ne, listenJSEvent as Be, NOOP as G, downloadFileFromBlob as Rt } from "@ibiz-template/core";
import { ElMessageBox as yt } from "element-plus";
import { debounce as bt, toNumber as Dt } from "lodash-es";
class Ct {
  constructor(t, e) {
    /**
     * 模态实例
  ...
  calcModalPosition() {
    ...
    if (!["bgColor", "color"].includes(d) || !n || !i || !l || u.getAttribute("data-w-e-toolbar") === "true")
      return;
...
const wt = /* @__PURE__ */ ke({
  ...
  setup(a, {
    ...
    const ie = {
      ...
      if (y.value)
        ...
        if (x)
          for (const H of x) {
            const [o] = H;
            o.type === "image" && v.endsWith(o.src) && V(o.src);
          }
      }
    }, V = async (c) => {
    ...
    const Te = (c) => {
      ...
      s.onDestroyed(), s.evt.off("setHtml", K), s.evt.off("clear", X), s.evt.off("onSetReply", ae);
    }, He = () => {
  ...
  render() {
    return this.isFullScreen ? p(q("el-dialog"), {
  ...
  setup(a, {
      ...
      if (s.deService) {
        ...
        const T = E.create({
          question: async ($) => {
            var nt;
            const R = Se();
            return await ((nt = s.deService) == null ? void 0 : nt.aiChatSse((D) => {
              if (ibiz.log.info("aiChatSse", D), D.actionstate === 20 && D.actionresult)
                T.addMessage({
                  messageid: R,
                  state: D.actionstate,
                  type: "DEFAULT",
                  role: "ASSISTANT",
                  content: D.actionresult
                });
              else if (D.actionstate === 30 && D.actionresult) {
                const St = JSON.parse(D.actionresult), {
                  choices: Ye
                } = St;
                ...
                D.actionstate === 40 && T.replaceMessage({
                  messageid: R,
                  state: D.actionstate,
                  type: "ERROR",
                  role: "ASSISTANT",
          ...
          action: ($, R) => {
            $ === "backfill" && (y.value ? M.value = R.content : t("change", R.content), K.value = !0);
          },
          closed: () => {
    ...
    s.chatCompletion && (V.insertKeys = {
      index: 60,
      keys: ["|", "aichart"]
    }), s.insertKeys.length > 0 && V.toolbarKeys && s.insertKeys.forEach((o) => {
      o.keys && o.keys.forEach((h, E) => {
        V.toolbarKeys.includes(h) || V.toolbarKeys.splice(o.index + E, 0, h);
      });
    });
    const xe = {
      placeholder: s.placeHolder,
    ...
    const he = async (o) => {
      ...
      f.value = o, s.onCreated(f.value, a.data, V);
...
const gs = /* @__PURE__ */ ke({
  ...
  render() {
    return ct(p("div", {
      ...
      class: this.ns.b()
    }, [this.items.map((a) => this.renderItem(a)), this.items.length === 0 && p(q("iBizNoData"), {
      text: "暂未用户数据"
...
class bs {
  ...
  handlePublicParams(t, e, s) {
    ...
    let u = {};
    return i && t && (u = De(i, t, s, e)), { context: d, params: u };
  }
  /**
   * 获取工作项信息
   *
...
class Xs extends Lt {
  ...
  async onInit() {
    ...
    if (e.appDEACModeId && (this.deACMode = await At(
      ...
      i && (this.minHeight = Number(i)), l && (this.maxHeight = Number(l)), d && (this.replyScript = d), u && (this.mode = u.toLowerCase()), f && (this.insertKeys = JSON.parse(f)), m && (this.renderMode = m), S && (this.saveInterval = Dt(S)), k && (this.emitMode = k), M && (this.collapsed.value = !Object.is(M, "TRUE") && !Object.is(M, "true")), F && (this.enableRealtime = Object.is(F, "TRUE") || Object.is(F, "true"));
    }
  ...
  registerCustomElem() {
    window.customElements.get("mention-elem") || window.customElements.define("mention-elem", Bs), window.customElements.get("personnel-marker-elem") || window.customElements.define(
      "personnel-marker-elem",
      $s
    ), window.paintFormatIsRegiter || (z.registerMenu(Vs), window.paintFormatIsRegiter = !0), window.personnelMarkerIsRegiter || (z.registerModule(Qs), window.personnelMarkerIsRegiter = !0), z.registerRenderStyle((e, s) => $t(e, s)), z.registerPlugin(Ys), window.aichartRegister || (z.registerMenu({
      key: "aichart",
      factory() {
        return new Gs();
      }
   ...
   */
  onCreated(e, s, n) {
    this.editor = e, this.onLineEditing(e, s), Q.getPluginsById(this.uuid).forEach((l) => {
      l.init(e, {
        model: this.model,
        data: s,
        toolbarConfig: n,
