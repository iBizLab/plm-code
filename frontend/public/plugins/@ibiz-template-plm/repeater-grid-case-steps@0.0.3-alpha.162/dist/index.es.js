    ...
    return {
      ...
      onChange: (c) => {
        ...
        if (c.moved) {
          ...
          F.group_id ? x[0].group_id = F.group_id : delete x[0].group_id, l.splice(v, 0, ...x), o("dragChange", l);
        }
      },
      renderRows: (c, l) => {
        var f, v, F, x;
  ...
  setup(r) {
    const o = T("repeater-grid"), t = [], e = r.controller, n = W().proxy, u = Y(() => {
      ...
      return e.value && e.value.forEach((m, g) => {
        m[e.groupField] || !g || !m[e.parentField] ? (a++, d = 0, s.push("".concat(a))) : (d++, s.push("".concat(a, ".").concat(d)));
      }), s;
    });
    ...
    const w = (s) => r.controller.enableDelete ? ibiz.config.form.mdCtrlConfirmBeforeRemove ? i(p("el-popconfirm"), {
      ...
      dropdown: () => {
        let s;
        return i(p("el-dropdown-menu"), null, P(s = e.presetAddAction.map((a) => i(p("el-dropdown-item"), {
          command: a.value
        }, {
          default: () => [i(p("ion-icon"), {
      ...
      const {
        row: a,
        $index: d
      } = s, m = a[e.groupField] == 1, g = !!a[e.parentField], h = e.getActionsByType(m, g) || [];
      return i(p("el-dropdown"), {
        ...
        dropdown: () => {
          let y;
          return i(p("el-dropdown-menu"), null, P(y = h.map((b) => i(p("el-dropdown-item"), {
            command: {
              actionType: b.value,
              index: d,
              row: a
      ...
      renderContent: () => {
        ...
        return t.forEach((a) => {
          a.hidden || (s[a.id] = (d) => {
            ...
            const h = g.formItems.find((K) => K.name === a.id);
            if (!h.state.visible)
              return null;
            const {
              width: y
            } = h.model, b = {};
            y && (b.width = "".concat(y, "px"), b.flex = "none");
            let S = null;
            if (!h.editorProvider)
              ...
              const K = p(h.editorProvider.formEditor);
              S = ne(K, {
                value: h.value,
                data: h.data,
                controller: h.editor,
