...
import { defineComponent as ie, ref as R, computed as P, onMounted as ce, onBeforeUnmount as de, createVNode as o, resolveComponent as i, createTextVNode as D } from "vue";
...
class he extends se {
  ...
  async onDragChange(t, h) {
    ...
    const p = this.model.minorSortDir === "ASC", { from: u, to: S, fromIndex: x, toIndex: B } = t, k = this.model.groupAppDEFieldId, f = this.model.minorSortAppDEFieldId, v = [];
    let I = this.state.groups.find((a) => a.key === u), m = this.state.groups.find((a) => a.key === S);
    if (I || (I = this.splitMap.get(u)), m || (m = this.splitMap.get(S)), !this.enableEditOrder) {
      if (t.from === t.to) {
        ibiz.message.warning("当前看板不允许调整次序");
        return;
      }
      const a = I.children[x];
      return a[k] = t.to, this.updateChangedItems([a]);
    }
    const g = (a) => a + (100 - a % 100), w = [...m.children], G = [...m.children], F = I.children[x];
    if (t.from !== t.to) {
      let a = t.to;
      h && (a = t.to.split("_")[0]), F[k] = a, G.splice(B, 0, F);
    } else
      G.splice(x, 1), G.splice(B, 0, F);
    const T = I.children.splice(x, 1);
    m.children.splice(B, 0, ...T), p || (w.reverse(), G.reverse());
    let b, A = [], $;
    G.forEach((a, y) => {
      b === void 0 ? (a !== w[y] || h && y === x) && (w[y] ? b = w[y][f] || 100 : b = y === 0 ? 100 : g(w[y - 1][f]), a[f] = b, v.push(a)) : (b >= a[f] ? (A.length > 0 && (A.forEach((K) => {
        K[f] = g($), v.push(K), $ = K[f];
...
const j = /* @__PURE__ */ ie({
  ...
  setup(d) {
    var U, q, W;
    const n = oe((...e) => new he(...e)), t = le("control-".concat(n.model.controlType.toLowerCase())), h = R(), p = R(!1), u = P(() => !n.state.draggable || n.state.updating), S = P(() => n.context.srfreadonly === !0 || n.context.srfreadonly === "true"), x = P(() => n.state.batching ? n.state.selectGroupKey : ""), B = (U = n.model.controls) == null ? void 0 : U.find((e) => e.name === "".concat(n.model.name, "_quicktoolbar") || e.name === "".concat(n.model.name, "_groupquicktoolbar")), k = (q = n.model.controls) == null ? void 0 : q.find((e) => e.name === "".concat(n.model.name, "_batchtoolbar")), f = ((W = n.model.groupSysCss) == null ? void 0 : W.cssName) || "", v = R({});
    let I = V;
    const m = {};
    ...
    const $ = (e, l) => {
      ...
      }, [D("已选择"), o("span", null, [l.length]), D("条数据")])]);
    }, J = (e) => {
      if (x.value === e.key)
        return o("div", {
          class: t.b("batch")
    ...
    const z = (e, l, r, s) => {
      ...
      return x.value === e.key ? o("span", {
        ...
        class: t.be("group", "header-right"),
        onClick: (r) => g(r)
      }, [n.enableNew && !S.value && o(i("el-button"), {
