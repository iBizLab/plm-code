...
import { useControlController as x, useNamespace as P, withInstall as v } from "@ibiz-template/vue3-util";
import { GridController as S, registerControlProvider as D, ControlType as G } from "@ibiz-template/runtime";
import { defineComponent as B, ref as z, watch as N, computed as b, createVNode as l, resolveComponent as u, isVNode as R } from "vue";
class w extends S {
  /**
   * 简单列表控制器
   *
...
const m = /* @__PURE__ */ B({
  ...
  setup() {
    const e = x((...t) => new w(...t)), n = P("checkbox-grid"), s = z([]);
    N(() => e.state.selectedData, (t) => {
      s.value = t.map((o) => o.srfkey);
    });
    const i = (t) => {
      !t || t === e.state.curPage || (e.state.curPage = t, e.load());
    }, h = (t) => {
      !t || t === e.state.size || (e.state.size = t, e.load());
    }, f = () => {
      e.load();
    }, c = b(() => {
      const {
    ...
    return {
      ...
      renderContent: () => {
        if (c.value.length > 0) {
          ...
          }, L(t = c.value.map((o) => l(u("el-checkbox"), {
            label: o.srfkey
          }, {
            default: () => [l("span", {
