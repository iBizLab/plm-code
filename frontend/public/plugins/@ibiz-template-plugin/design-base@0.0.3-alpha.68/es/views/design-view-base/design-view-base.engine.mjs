...
import { RuntimeModelError, HttpError } from '@ibiz-template/core';
...
class DesignViewEngineBase extends ViewEngineBase {
  ...
  initViewShouldDismissHook() {
    this.view.modal.hooks.shouldDismiss.tapPromise(async (context) => {
      ...
      if (uiDomain && uiDomain.dataModification === true && context.allowClose == null) {
  ...
  async load() {
    try {
      ...
      if (error instanceof HttpError) {
        error.tag = this.view.context.srfviewid;
      }
      throw error;
    } finally {
    ...
    try {
      ...
      if (majorResult.ok && majorResult.data) {
        ...
        if (res.ok && res.data) {
          ...
          this.view.evt.emit("onSaveSuccess", void 0);
