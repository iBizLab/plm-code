export default {
  codeListTag: 'ProjMgmt__work_item_advanced_search_field',
  codeListType: 'STATIC',
  codeName: 'ProjMgmt__work_item_advanced_search_field',
  emptyText: '未定义',
  appDataEntityId: 'plmweb.work_item',
  codeItems: [
    {
      codeName: 'n_title_like',
      text: '标题',
      value: 'n_title_like',
      id: 'n_title_like',
    },
    {
      codeName: 'n_identifier_like',
      text: '编号',
      value: 'n_identifier_like',
      id: 'n_identifier_like',
    },
    {
      codeName: 'n_description_like',
      text: '描述',
      value: 'n_description_like',
      id: 'n_description_like',
    },
  ],
  enableCache: true,
  name: '工作项高级搜索属性',
  id: 'plmweb.projmgmt__work_item_advanced_search_field',
};
