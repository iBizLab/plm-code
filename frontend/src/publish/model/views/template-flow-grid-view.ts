export default {
  gridRowActiveMode: 1,
  enableRowEdit: true,
  rowEditDefault: true,
  xdataControlName: 'grid',
  loadDefault: true,
  deviewCodeName: 'template_flow_grid_view',
  deviewId: '7df66424898fa7af70df389a3563f68a',
  accUserMode: 2,
  caption: '自动化规则',
  codeName: 'template_flow_grid_view',
  appDataEntityId: 'plmweb.psdelogic',
  appViewEngines: [
    {
      engineCat: 'VIEW',
      engineType: 'GridView',
      id: 'engine',
    },
  ],
  appViewLogics: [
    {
      logicTrigger: 'CUSTOM',
      logicType: 'APPUILOGIC',
      builtinAppUILogic: {
        actionAfterWizard: 'DEFAULT',
        newDataAppView: {
          refAppViewId: 'plmweb.psdelogiceditview',
        },
        appUILogicRefViews: [
          {
            refAppViewId: 'plmweb.psdelogiceditview',
          },
        ],
        builtinLogic: true,
        logicType: 'PREDEFINED',
        viewLogicType: 'APP_NEWDATA',
        id: '新建数据',
      },
      builtinLogic: true,
      id: 'newdata',
    },
    {
      logicTrigger: 'CUSTOM',
      logicType: 'APPUILOGIC',
      builtinAppUILogic: {
        openDataAppView: {
          openMode: 'POPUPMODAL',
          refAppViewId: 'plmweb.psdelogicflow_main_view',
        },
        editMode: true,
        appUILogicRefViews: [
          {
            openMode: 'POPUPMODAL',
            refAppViewId: 'plmweb.psdelogicflow_main_view',
          },
        ],
        builtinLogic: true,
        logicType: 'PREDEFINED',
        viewLogicType: 'APP_OPENDATA',
        id: '打开数据',
      },
      builtinLogic: true,
      id: 'opendata',
    },
  ],
  appViewNavContexts: [
    {
      rawValue: true,
      key: 'PSDEID',
      value: 'Base.template_flow',
      name: 'PSDEID',
      id: 'psdeid',
    },
  ],
  appViewNavParams: [
    {
      rawValue: true,
      key: 'n_psdeid_eq',
      value: 'Base.template_flow',
      id: 'n_psdeid_eq',
    },
    {
      rawValue: true,
      key: 'n_dynamodelflag_eq',
      value: '1',
      id: 'n_dynamodelflag_eq',
    },
  ],
  appViewRefs: [
    {
      realOpenMode: 'POPUPMODAL',
      realTitle: '自动化规则',
      refAppViewId: 'plmweb.psdelogicflow_main_view',
      name: 'EDITDATA',
      id: 'editdata',
    },
    {
      realTitle: '实体处理逻辑编辑视图',
      refAppViewId: 'plmweb.psdelogiceditview',
      name: 'NEWDATA',
      id: 'newdata',
    },
  ],
  controls: [
    {
      detoolbarItems: [
        {
          actionLevel: 100,
          detoolbarItems: [
            {
              actionLevel: 100,
              noPrivDisplayMode: 2,
              uiactionId: 'create_auto_flow@psdelogic',
              uiactionTarget: 'NONE',
              valid: true,
              caption: '自动化规则',
              itemType: 'DEUIACTION',
              tooltip: '自动化规则',
              showCaption: true,
              showIcon: true,
              id: 'deuiaction1',
            },
            {
              actionLevel: 100,
              noPrivDisplayMode: 2,
              uiactionId: 'create_schedule_flow@psdelogic',
              uiactionTarget: 'NONE',
              valid: true,
              caption: '计划规则',
              itemType: 'DEUIACTION',
              tooltip: '计划规则',
              showCaption: true,
              showIcon: true,
              id: 'deuiaction2',
            },
          ],
          valid: true,
          caption: '新建规则',
          itemType: 'ITEMS',
          sysImage: {
            cssClass: 'fa fa-plus',
            glyph: 'xf067@FontAwesome',
          },
          tooltip: '新建规则',
          showCaption: true,
          showIcon: true,
          id: 'items1',
        },
      ],
      xdataControlName: 'grid',
      codeName: 'template_flow_grid_view_toolbar',
      controlType: 'TOOLBAR',
      logicName: '自动化规则逻辑表格视图_工具栏',
      appDataEntityId: 'plmweb.psdelogic',
      controlParam: {
        id: 'toolbar',
      },
      modelId: '1fa77497c4df98ee205ec259309b0a23',
      modelType: 'PSDETOOLBAR',
      name: 'toolbar',
      id: 'template_flow_grid_view_toolbar',
    },
    {
      aggMode: 'NONE',
      columnEnableFilter: 2,
      columnEnableLink: 2,
      groupMode: 'NONE',
      groupStyle: 'DEFAULT',
      orderValueAppDEFieldId: 'ordervalue',
      degridColumns: [
        {
          clconvertMode: 'NONE',
          dataItemName: 'psdelogicname',
          excelCaption: '名称',
          appDEFieldId: 'psdelogicname',
          deuiactionGroup: {
            uiactionGroupDetails: [
              {
                actionLevel: 200,
                afterItemType: 'NONE',
                beforeItemType: 'NONE',
                detailType: 'DEUIACTION',
                uiactionId: 'apply@psdelogic',
                tooltip: '应用此规则',
                showIcon: true,
                sysImage: {
                  cssClass: 'fa fa-arrow-circle-o-right',
                  glyph: 'xf18e@FontAwesome',
                },
                id: 'u3715cc4',
              },
              {
                actionLevel: 100,
                afterItemType: 'NONE',
                beforeItemType: 'NONE',
                caption: '编辑',
                detailType: 'DEUIACTION',
                uiactionId: 'open_quick_cfg_view@psdelogic',
                showCaption: true,
                showIcon: true,
                sysImage: {
                  cssClass: 'fa fa-edit',
                  glyph: 'xf044@FontAwesome',
                },
                id: 'uc1356a2',
              },
              {
                actionLevel: 100,
                afterItemType: 'NONE',
                beforeItemType: 'NONE',
                capLanguageRes: {
                  lanResTag: 'TBB.TEXT.*.REMOVE',
                },
                caption: '删除',
                detailType: 'DEUIACTION',
                uiactionId: 'remove',
                tooltip: '删除',
                tooltipLanguageRes: {
                  lanResTag: 'TBB.TOOLTIP.*.REMOVE',
                },
                showCaption: true,
                showIcon: true,
                sysImage: {
                  cssClass: 'fa fa-trash-o',
                  glyph: 'xf014@FontAwesome',
                },
                id: 'u043ffc0',
              },
            ],
            appDataEntityId: 'plmweb.psdelogic',
            uniqueTag: 'PSDELogic__Usr0118075954',
            name: '自动化规则表格列行为组',
            id: 'usr0118075954',
          },
          valueType: 'SIMPLE',
          aggMode: 'NONE',
          align: 'LEFT',
          caption: '名称',
          codeName: 'psdelogicname',
          columnType: 'DEFGRIDCOLUMN',
          noPrivDisplayMode: 1,
          width: 250,
          widthUnit: 'STAR',
          enableSort: true,
          id: 'psdelogicname',
        },
        {
          clconvertMode: 'FRONT',
          dataItemName: 'logicsubtype',
          excelCaption: '类型',
          appCodeListId: 'plmweb.extension__delogicsubtype',
          appDEFieldId: 'logicsubtype',
          valueType: 'SIMPLE',
          aggMode: 'NONE',
          align: 'LEFT',
          caption: '类型',
          codeName: 'logicsubtype',
          columnType: 'DEFGRIDCOLUMN',
          noPrivDisplayMode: 1,
          width: 150,
          widthUnit: 'PX',
          enableSort: true,
          id: 'logicsubtype',
        },
        {
          clconvertMode: 'FRONT',
          dataItemName: 'validflag',
          excelCaption: '启用',
          appCodeListId: 'plmweb.yesno',
          appDEFieldId: 'validflag',
          valueType: 'SIMPLE',
          enableRowEdit: true,
          aggMode: 'NONE',
          align: 'LEFT',
          caption: '启用',
          codeName: 'validflag',
          columnType: 'DEFGRIDCOLUMN',
          noPrivDisplayMode: 1,
          width: 100,
          widthUnit: 'PX',
          enableSort: true,
          id: 'validflag',
        },
        {
          clconvertMode: 'FRONT',
          dataItemName: 'applyflag',
          excelCaption: '状态',
          appCodeListId: 'plmweb.extension__extension_status',
          appDEFieldId: 'applyflag',
          valueType: 'SIMPLE',
          aggMode: 'NONE',
          align: 'LEFT',
          caption: '状态',
          codeName: 'applyflag',
          columnType: 'DEFGRIDCOLUMN',
          noPrivDisplayMode: 1,
          width: 100,
          widthUnit: 'PX',
          enableSort: true,
          id: 'applyflag',
        },
        {
          clconvertMode: 'NONE',
          dataItemName: 'createdate',
          excelCaption: '建立时间',
          appDEFieldId: 'createdate',
          valueFormat: 'YYYY-MM-DD HH:mm:ss',
          valueType: 'SIMPLE',
          aggMode: 'NONE',
          align: 'LEFT',
          capLanguageRes: {
            lanResTag: 'DEF.LNAME.CREATEDATE',
          },
          caption: '建立时间',
          codeName: 'createdate',
          columnType: 'DEFGRIDCOLUMN',
          noPrivDisplayMode: 1,
          width: 200,
          widthUnit: 'PX',
          enableSort: true,
          id: 'createdate',
        },
      ],
      degridDataItems: [
        {
          appDEFieldId: 'psdelogicname',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'psdelogicname',
        },
        {
          appDEFieldId: 'logicsubtype',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'logicsubtype',
        },
        {
          appDEFieldId: 'validflag',
          valueType: 'SIMPLE',
          dataType: 9,
          id: 'validflag',
        },
        {
          appDEFieldId: 'applyflag',
          valueType: 'SIMPLE',
          dataType: 9,
          id: 'applyflag',
        },
        {
          format: 'YYYY-MM-DD HH:mm:ss',
          appDEFieldId: 'createdate',
          valueType: 'SIMPLE',
          dataType: 5,
          id: 'createdate',
        },
        {
          appDEFieldId: 'events',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'events',
        },
        {
          appDEFieldId: 'createman',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'createman',
        },
        {
          appDEFieldId: 'updateman',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'updateman',
        },
        {
          format: 'YYYY-MM-DD HH:mm:ss',
          appDEFieldId: 'updatedate',
          valueType: 'SIMPLE',
          dataType: 5,
          id: 'updatedate',
        },
        {
          appDEFieldId: 'psdelogicid',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'srfkey',
        },
        {
          appDEFieldId: 'psdelogicid',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'srfdataaccaction',
        },
        {
          appDEFieldId: 'psdelogicname',
          valueType: 'SIMPLE',
          dataType: 25,
          id: 'srfmajortext',
        },
      ],
      degridEditItems: [
        {
          caption: '启用',
          codeName: 'validflag',
          enableCond: 3,
          appDEFieldId: 'validflag',
          editor: {
            editorType: 'SWITCH',
            valueType: 'SIMPLE',
            editable: true,
            id: 'validflag',
          },
          allowEmpty: true,
          id: 'validflag',
        },
        {
          caption: '实体处理逻辑标识',
          codeName: 'srfkey',
          enableCond: 3,
          appDEFieldId: 'psdelogicid',
          editor: {
            editorType: 'HIDDEN',
            valueType: 'SIMPLE',
            editable: true,
            id: 'srfkey',
          },
          allowEmpty: true,
          id: 'srfkey',
        },
      ],
      pagingMode: 1,
      pagingSize: 20,
      sortMode: 'REMOTE',
      enableCustomized: true,
      enablePagingBar: true,
      enableRowEdit: true,
      createControlAction: {
        appDEMethodId: 'create',
        appDataEntityId: 'plmweb.psdelogic',
        id: 'create',
      },
      fetchControlAction: {
        appDEMethodId: 'fetchdefault',
        appDataEntityId: 'plmweb.psdelogic',
        id: 'fetch',
      },
      getDraftControlAction: {
        appDEMethodId: 'getdraft',
        appDataEntityId: 'plmweb.psdelogic',
        id: 'loaddraft',
      },
      getControlAction: {
        appDEMethodId: 'get',
        appDataEntityId: 'plmweb.psdelogic',
        id: 'load',
      },
      removeControlAction: {
        appDEMethodId: 'remove',
        appDataEntityId: 'plmweb.psdelogic',
        id: 'remove',
      },
      updateControlAction: {
        appDEMethodId: 'update',
        appDataEntityId: 'plmweb.psdelogic',
        id: 'update',
      },
      autoLoad: true,
      showBusyIndicator: true,
      codeName: 'flow_grid_view_grid',
      controlType: 'GRID',
      logicName: '自动化规则逻辑表格视图_表格',
      appDataEntityId: 'plmweb.psdelogic',
      controlLogics: [
        {
          eventNames: 'onSaveSuccess',
          logicTag: 'grid',
          logicType: 'APPDEUIACTION',
          appDEUIActionId: 'refresh',
          triggerType: 'CTRLEVENT',
          id: 'refresh',
        },
      ],
      controlParam: {
        id: 'grid',
      },
      modelId: 'eb868b0671ad0861a3293f856c9830c1',
      modelType: 'PSDEGRID',
      name: 'grid',
      id: 'plmweb.psdelogic.flow_grid_view_grid',
    },
    {
      searchButtonStyle: 'DEFAULT',
      deformPages: [
        {
          layout: {
            columnCount: 24,
            layout: 'TABLE_24COL',
          },
          caption: '常规条件',
          codeName: 'formpage1',
          detailStyle: 'DEFAULT',
          detailType: 'FORMPAGE',
          id: 'formpage1',
        },
      ],
      layout: {
        columnCount: 24,
        layout: 'TABLE_24COL',
      },
      tabHeaderPos: 'TOP',
      noTabHeader: true,
      autoLoad: true,
      showBusyIndicator: true,
      codeName: 'flow_grid_view_search_form',
      controlType: 'SEARCHFORM',
      logicName: '自动化规则逻辑表格视图_搜索表单',
      appDataEntityId: 'plmweb.psdelogic',
      controlParam: {
        id: 'searchform',
      },
      modelId: '7ba4a133f64037d9976896af38072334',
      modelType: 'PSDEFORM_SEARCHFORM',
      name: 'searchform',
      id: 'plmweb.psdelogic.flow_grid_view_search_form',
    },
    {
      groupMode: 'SINGLE',
      quickSearchMode: 1,
      enableQuickSearch: true,
      controlType: 'SEARCHBAR',
      appDataEntityId: 'plmweb.psdelogic',
      controlParam: {
        id: 'searchbar',
      },
      id: 'searchbar',
    },
    {
      caption: '自动化规则',
      codeName: 'template_flow_grid_view_captionbar',
      controlType: 'CAPTIONBAR',
      appDataEntityId: 'plmweb.psdelogic',
      controlParam: {},
      name: 'captionbar',
      id: 'template_flow_grid_view_captionbar',
    },
  ],
  viewLayoutPanel: {
    layoutBodyOnly: true,
    useDefaultLayout: true,
    layoutPanel: true,
    codeName: 'layoutpanel',
    controlStyle: 'APPDEGRIDVIEW',
    controlType: 'VIEWLAYOUTPANEL',
    appDataEntityId: 'plmweb.psdelogic',
    controlParam: {},
    id: 'layoutpanel',
  },
  title: '自动化规则',
  viewStyle: 'DEFAULT',
  viewType: 'DEGRIDVIEW',
  enableDP: true,
  showCaptionBar: true,
  modelId: '6714071f4a59cc86696b49c52b5b0e37',
  modelType: 'PSAPPDEVIEW',
  name: 'template_flow_grid_view',
  id: 'plmweb.template_flow_grid_view',
};
