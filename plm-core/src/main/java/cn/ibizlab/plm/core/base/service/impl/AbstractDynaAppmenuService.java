...
public abstract class AbstractDynaAppmenuService extends ServiceImpl<DynaAppmenuMapper,DynaAppmenu> implements DynaAppmenuService {
   ...
   @Transactional
    ...
    public CheckKeyStatus checkKey(DynaAppmenu et) {
        return (!ObjectUtils.isEmpty(et.getId()) && this.count(Wrappers.<DynaAppmenu>lambdaQuery().eq(DynaAppmenu::getId, et.getId()))>0)? CheckKeyStatus.FOUNDED : CheckKeyStatus.NOT_FOUND;
    }
   ...
   public Page<DynaAppmenu> fetchView(DynaAppmenuSearchContext context) {
        com.baomidou.mybatisplus.extension.plugins.pagination.Page<DynaAppmenu> pages=baseMapper.searchView(context.getPages(),context,context.getSelectCond());
        List<DynaAppmenu> list = pages.getRecords();
        return new PageImpl<>(list, context.getPageable(), pages.getTotal());
    }

   public List<DynaAppmenu> listView(DynaAppmenuSearchContext context) {
        List<DynaAppmenu> list = baseMapper.listView(context,context.getSelectCond());
        return list;
   }
