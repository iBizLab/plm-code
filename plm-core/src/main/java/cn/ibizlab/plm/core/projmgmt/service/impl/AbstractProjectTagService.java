...
public abstract class AbstractProjectTagService extends ServiceImpl<ProjectTagMapper,ProjectTag> implements ProjectTagService {
   ...
   @Transactional
    ...
    public CheckKeyStatus checkKey(ProjectTag et) {
        return (!ObjectUtils.isEmpty(et.getId()) && this.count(Wrappers.<ProjectTag>lambdaQuery().eq(ProjectTag::getId, et.getId()))>0)? CheckKeyStatus.FOUNDED : CheckKeyStatus.NOT_FOUND;
    }
   ...
   public Page<ProjectTag> fetchView(ProjectTagSearchContext context) {
        com.baomidou.mybatisplus.extension.plugins.pagination.Page<ProjectTag> pages=baseMapper.searchView(context.getPages(),context,context.getSelectCond());
        List<ProjectTag> list = pages.getRecords();
        return new PageImpl<>(list, context.getPageable(), pages.getTotal());
    }

   public List<ProjectTag> listView(ProjectTagSearchContext context) {
        List<ProjectTag> list = baseMapper.listView(context,context.getSelectCond());
        return list;
   }
