...
public abstract class AbstractUpdateRecordService extends ServiceImpl<UpdateRecordMapper,UpdateRecord> implements UpdateRecordService {
   ...
   @Transactional
    ...
    public Integer checkKey(UpdateRecord et) {
        return (!ObjectUtils.isEmpty(et.getId()) && this.count(Wrappers.<UpdateRecord>lambdaQuery().eq(UpdateRecord::getId, et.getId()))>0)?1:0;
    }
   ...
   public List<UpdateRecord> listDefault(UpdateRecordSearchContext context) {
	...
	public boolean saveByKeyResult(KeyResult keyResult, List<UpdateRecord> list){
        if(list==null)
            return true;
        Map<String,UpdateRecord> before = findByKeyResultId(keyResult.getId()).stream().collect(Collectors.toMap(UpdateRecord::getId,e->e));
	...
	public boolean saveByObjective(Objective objective, List<UpdateRecord> list){
        if(list==null)
            return true;
        Map<String,UpdateRecord> before = findByObjectiveId(objective.getId()).stream().collect(Collectors.toMap(UpdateRecord::getId,e->e));
